document.addEventListener('DOMContentLoaded', function() {
    ...
    // REMOVIDO: const disciplinaSelect = document.getElementById('disciplina'); // Esta linha é desnecessária e incorreta para um radio_group
    ...
    const disciplinaRadios = document.querySelectorAll('input[name="disciplina"]'); // Correto: referencia os rádios de disciplina

    disciplinaRadios.forEach(radio => {
        radio.addEventListener('change', function() {
            const selectedDisciplinaId = this.value; // Obtém o valor do rádio selecionado
            
            // Ocultar e resetar todos os assuntos e "Selecionar Todos"
            hideAndResetGroup(assuntosContainer, selecionarTodosAssuntosLabel, assuntoCheckboxes, selecionarTodosAssuntosCheckbox);
            // Ao mudar a disciplina, resetar e ocultar Formatos de Perguntas e Subtipos RA (importante para dependências em cascata)
            hideAndResetGroup(formatosPerguntasContainer, selecionarTodosFormatosPerguntasLabel, formatoPerguntaCheckboxes, selecionarTodosFormatosPerguntasCheckbox);
            hideAndResetGroup(subtiposRespostaAbertaContainer, selecionarTodosSubtiposRespostaAbertaLabel, subtiposRespostaAbertaCheckboxes, selecionarTodosSubtiposRespostaAbertaCheckbox);
    ...
    if (selecionarTodosAssuntosCheckbox) {
        selecionarTodosAssuntosCheckbox.addEventListener('change', function() {
            const isChecked = this.checked;
            const selectedDisciplinaId = document.querySelector('input[name="disciplina"]:checked')?.value; // Correto
    ...
    function updateFormatosPerguntasVisibility() {
        // CORRIGIDO: Obtenha o ID da disciplina selecionada diretamente dos rádios
        const selectedDisciplinaId = document.querySelector('input[name="disciplina"]:checked')?.value;
        
        let anyAssuntoSelected = false;
        
        // REMOVIDO: O trecho abaixo era redundante e poderia causar recursão
        /*
        ...
        */
        ...
        if (selectedDisciplinaId) {
            ...
            assuntoCheckboxes.forEach(checkbox => {
                // Verifica se o checkbox pertence à disciplina selecionada E está marcado
    ...
    function updateSubtiposRespostaAbertaVisibility() {
        ...
        formatoPerguntaCheckboxes.forEach(checkbox => {
            // Verifica se o valor do checkbox de formato de pergunta inclui "resposta aberta" (case insensitive) E está marcado
    ...
    // CORRIGIDO: Obtenha o rádio de disciplina selecionado, não um elemento por ID
    ...
    if (initialSelectedDisciplinaRadio) {
        ...
        // Se nenhuma disciplina estiver selecionada inicialmente, garante que Assuntos, Formatos e Subtipos RA estejam ocultos
        hideAndResetGroup(assuntosContainer, selecionarTodosAssuntosLabel, assuntoCheckboxes, selecionarTodosAssuntosCheckbox);
        hideAndResetGroup(formatosPerguntasContainer, selecionarTodosFormatosPerguntasLabel, formatoPerguntaCheckboxes, selecionarTodosFormatosPerguntasCheckbox);
        hideAndResetGroup(subtiposRespostaAbertaContainer, selecionarTodosSubtiposRespostaAbertaLabel, subtiposRespostaAbertaCheckboxes, selecionarTodosSubtiposRespostaAbertaCheckbox);
    }
    ...
    // Já tratadas acima, mas manter para clareza se houver outras inicializações
    /*
