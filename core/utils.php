...
function validar_dados_server_side(
        array $dados_recebidos, // $_POST,
        array $configuracoes, // $config_json,
        array $turmas_info, // $turmas_info_completa,
        array $disciplina_assuntos_info, // $disciplina_assuntos_info_completa,
        array $formatos_disponiveis // $formatos_perguntas_disponiveis
        ): array 
    {
        $erros = [];
        $dados_validados = [];
            ...
            // --- VALIDAÇÃO DE ASSUNTOS ---
                if ($disciplina_valida && $disciplina_selecionada_detalhes) {
                    ...
                    if (empty($assuntos_selecionados) || !is_array($assuntos_selecionados)) {
                        // Se nenhum assunto foi selecionado, mas não é um erro fatal, apenas não adiciona ao dados_validados
                    } else {
                        ...
                        if (empty($assuntos_validos_da_disciplina) && !empty($assuntos_selecionados)) {
                            $erros[] = "Nenhum assunto válido foi selecionado para a disciplina " . htmlspecialchars($disciplina_selecionada_detalhes["nome"]) . ".";
                        } else if (!empty($assuntos_validos_da_disciplina)) {
                            $dados_validados["assuntos"] = $assuntos_validos_da_disciplina;
                        }
                    }
                } else if (empty($disciplina_id_selecionada)) {
                    // Erro de disciplina já tratado
                } else {
                    $erros[] = "Não foi possível validar os assuntos. Disciplina inválida ou não encontrada.";
                }

            // ===================Formato das Perguntas
                $formatos_selecionados_post = $dados_recebidos["formatosPerguntas"] ?? [];
                $dados_validados["formatosPerguntas"] = []; // Inicializa como array vazio

                if (empty($formatos_selecionados_post) || !is_array($formatos_selecionados_post)) {
                    // Se nenhum formato foi selecionado, mas não é um erro fatal, apenas não adiciona ao dados_validados
                } else {
                    $formatos_validos_selecionados = [];
                    $resposta_aberta_selecionada = false;
                    foreach ($formatos_selecionados_post as $formato_bruto) {
                        $formato_sanitizado = sanitizar_dados($formato_bruto);
                        if (array_key_exists($formato_sanitizado, $formatos_disponiveis)) {
                            $formatos_validos_selecionados[] = $formato_sanitizado;
                    ...
                    if (empty($formatos_validos_selecionados) && !empty($formatos_selecionados_post)) {
                        $erros[] = "Nenhum formato de pergunta válido foi selecionado.";
                    } else if (!empty($formatos_validos_selecionados)) {
                        $dados_validados["formatosPerguntas"] = $formatos_validos_selecionados;
                    }
